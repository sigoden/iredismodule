...
impl Context {
    pub fn get_blocked_client_ready_key(&self) -> Result<RStr, Error> {
        let p: *mut raw::RedisModuleString = unsafe { raw::RedisModule_GetBlockedClientReadyKey.unwrap()(self.inner) };
        if p.is_null() {
    ...
    {
        let mut keys: Vec<*mut raw::RedisModuleString> =
            keys.iter().map(|s| s.get_ptr()).collect();
    ...
    pub fn is_blocked_reply_request(&self) -> bool {
        let ret = unsafe {
            raw::RedisModule_IsBlockedReplyRequest.unwrap()(self.inner)
        };
        ret != 0
    }
    pub fn is_blocked_timeout_request(&self) -> bool {
        let ret = unsafe {
            raw::RedisModule_IsBlockedTimeoutRequest.unwrap()(self.inner)
        };
        ret != 0
    }
    pub fn get_block_client_private_data<T>(&self) -> &mut T {
        let data: *mut c_void = unsafe {
            raw::RedisModule_GetBlockedClientPrivateData.unwrap()(self.inner)
        };
        unsafe { &mut *(data as *mut T) }
    }
    pub fn get_block_client_handle(&self) -> BlockClient {
        let bc: *mut raw::RedisModuleBlockedClient = unsafe {
            raw::RedisModule_GetBlockedClientHandle.unwrap()(self.inner)
        };
        BlockClient::from_ptr(bc)
    }
    pub fn blocked_client_disconnected(&self) -> bool {
        let ret = unsafe {
            raw::RedisModule_BlockedClientDisconnected.unwrap()(self.inner)
        };
        ret != 0
    }
}